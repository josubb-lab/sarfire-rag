...
def detect_user_intention(message: str, scenario_active: bool) -> str:
    """
    Detecta la intención del usuario priorizando el estado de escenario.

    - Si hay escenario activo: por defecto interpretamos el mensaje como respuesta del usuario,
      salvo que pida explícitamente un nuevo escenario.
    - Si no hay escenario activo: si detectamos petición de escenario, generamos uno nuevo.
    """
    message_lower = message.lower().strip()

    # PRIORIDAD 1: Si hay escenario activo, solo generar nuevo si EXPLÍCITAMENTE pide otro
    if scenario_active:
        ...
        if explicit_new:
            print("🔍 DEBUG: Usuario pide NUEVO escenario (explícito)")
            return 'new_scenario'

        print("🔍 DEBUG: Usuario responde a escenario ACTIVO")
        return 'scenario_response'

    # PRIORIDAD 2: Si NO hay escenario activo, permitir keywords amplias para pedir escenario
    scenario_keywords = [
    ...
    try:
        ...
        if mode == "🎯 Automático (Director decide)":
            ...
            if hasattr(formador, 'rag_pipeline'):
                ...
                # Si checkbox activo → True (forzar búsqueda externa si relevancia baja)
                # Si checkbox inactivo → False (solo DTF-13, no buscar en web)
            ...
            if intention == 'scenario_response' and scenario_state.get("active"):
                ...
                # Evitar que el formateador muestre un "scenario" residual durante la evaluación
                ...
                result = simulador.create_scenario(
                    topic=message, 
                    allow_external=True if allow_external else None  # True fuerza, None pregunta
                )
