...
MVP principles:
- Explicit conversation state (scenario + pending external confirmation).
- Consistent external policy: never | ask | always.
- Director routes only when no flow is active.
- Web is only used to enrich (Simulador) or answer with disclaimer (Formador).
"""
...
def detect_user_intention(message: str, scenario_active: bool) -> str:
    """Priority: if scenario is active, default to scenario_response unless explicit new scenario request."""
    ...
    if result.get("source") == "external":
        ...
        if result.get("external_sources"):
            ...
            for i, src in enumerate(result["external_sources"][:3], 1):
                response += f"\n{i}. [{src.get('title','Sin título')}]({src.get('url','')})"
...
def process_message(
    ...
    try:
        ...
        if state["pending"]["active"]:
            ...
            if _is_yes(message):
                if kind == "formador":
                    rag_result = formador.rag.query(question=payload, allow_external=True)
                ...
                if kind == "formador":
                    rag_result = formador.rag.query(question=payload, allow_external=False)
                    # If low relevance, this will likely return internal answer or "no info"
                    response = format_formador_response(rag_result)
                    state["pending"] = {"active": False, "kind": None, "payload": None}
                    state["scenario"] = {"active": False, "text": None}
                else:
        ...
        # 3) Execute
        if target == "simulador":
            # Treat any request here as scenario creation for MVP
            sim_result = simulador.create_scenario(topic=message, allow_external=allow_external_mode)

            if sim_result.get("should_ask_user"):
                # Ask and set pending; do NOT activate scenario yet
                response = "**🎭 AGENTE SIMULADOR**\n\n" + sim_result.get("scenario", "")
                response += "\n\n" + sim_result.get("question_for_user", '¿Deseas enriquecer con web? Responde "sí" o "no".')
        ...
        state = {"scenario": {"active": False, "text": None}, "pending": {"active": False, "kind": None, "payload": None}}
