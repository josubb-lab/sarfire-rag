...
def detect_user_intention(message: str, scenario_active: bool) -> str:
    ...
    if scenario_active:
        ...
        if asking_new_scenario:
            print("🔍 DEBUG: Usuario pide NUEVO escenario (escenario activo ignorado)")
            return 'new_scenario'
        else:
    ...
    if any(kw in message_lower for kw in scenario_keywords):
        print("🔍 DEBUG: Usuario pide nuevo escenario (no hay activo)")
        return 'new_scenario'
...
def process_message(
    ...
    history: list, 
    scenario_state: dict,
    allow_external: bool,  # Checkbox: permite (con confirmación) búsquedas externas
    external_pending_state: dict  # Estado: pendiente de confirmar búsqueda externa
):
    """
    ...
    Returns:
        Tuple (historial, input vacío, scenario_state, allow_external, external_pending_state)
    """
    ...
    try:
        ...
        if mode == "🎯 Automático (Director decide)":
            ...
            # ----------------------------------------------------------------
            # FLUJO FORMADOR + BÚSQUEDA EXTERNA CON CONFIRMACIÓN
            #
            # Requisito:
            # - Si la relevancia interna (DTF-13) < umbral → avisar y preguntar si buscar en web.
            # - Si el usuario responde "sí" → ejecutar búsqueda externa y devolver con disclaimer.
            #
            # Checkbox (allow_external):
            # - False  => SOLO DTF-13 (allow_external=False)
            # - True   => Permitido, pero con confirmación (allow_external=None)
            # ----------------------------------------------------------------

            # 1) Si hay una confirmación pendiente, interpretar este mensaje como sí/no
            if external_pending_state.get("pending"):
                answer_lower = message.strip().lower()

                is_yes = bool(re.match(r"^(s[ií]|sí|si|vale|ok|okay|de acuerdo|adelante|hazlo|busca|buscar|procede)\b", answer_lower))
                is_no = bool(re.match(r"^(no|nah|mejor no|negativo|cancela|cancelar)\b", answer_lower))

                original_question = external_pending_state.get("question")

                if is_yes and original_question:
                    rag_result = formador.rag.query(
                        question=original_question,
                        allow_external=True
                    )
                    rag_result.pop("scenario", None)
                    ...
                    external_pending_state = {"pending": False, "question": None}

                elif is_no:
                    result = {
                        "answer": "De acuerdo. No haré búsqueda externa. Si quieres, reformula la pregunta o activa la opción de búsqueda externa para poder solicitarla.",
                        "source": "none",
                        "should_ask_user": False,
                        "question_for_user": None
                    }
                    ...
                    # No queda claro: repetir la pregunta de confirmación sin cambiar estado
                    result = {
                ...
                # 2) Ejecución normal (no hay confirmación pendiente)
                allow_external_mode = None if allow_external else False

                rag_result = formador.rag.query(
                    question=message,
                    allow_external=allow_external_mode
                )
                rag_result.pop("scenario", None)
                ...
                if result.get("should_ask_user"):
                    external_pending_state = {"pending": True, "question": message}
    ...
    # Mantener allow_external (checkbox) y devolver estado de confirmación externa
    return history, "", scenario_state, allow_external, external_pending_state


def clear_chat():
    """Limpia el historial del chat y el estado del escenario"""
    return [], "", {"text": None, "active": False}, False, {"pending": False, "question": None}
...
with gr.Blocks(title="SARFIRE-RAG v2.0") as demo:
    ...
    external_pending_state = gr.State({"pending": False, "question": None})
    ...
    submit_btn.click(
        fn=process_message,
        inputs=[msg, mode_selector, chatbot, scenario_state, external_checkbox, external_pending_state],
        outputs=[chatbot, msg, scenario_state, external_checkbox, external_pending_state]
    )
