...
class RAGPipeline:
    ...
    def __init__(
        ...
        relevance_threshold: float = 0.3  # ← NUEVO
    ):
        """
        Args:
    ...
    def assess_relevance(self, retrieved_chunks: List[Dict]) -> float:
        ...
        Nota:
        - Si el retrieve proviene de HybridSearch, prioriza 'hybrid_score' (score combinado).
        - Si no existe, utiliza 'similarity' (similitud semántica 0-1).

        Args:
            retrieved_chunks: Lista de chunks con 'similarity' y opcionalmente 'hybrid_score'

        Returns:
            Score de relevancia 0-1 (aprox). Valores más altos = más relevante.
        """
        ...
        for chunk in retrieved_chunks:
            if 'hybrid_score' in chunk:
                try:
                    scores.append(float(chunk['hybrid_score']))
                except Exception:
                    scores.append(0.0)
            else:
                try:
                    scores.append(float(chunk.get('similarity', 0.0)))
                except Exception:
                    scores.append(0.0)
...
RESPUESTA:"""
    ...
    def query(
        ...
        for i, c in enumerate(retrieved_chunks[:10], 1):
            meta = c.get("metadata", {})
            print(
                f"[{i}] file={meta.get('filename')} page={meta.get('page_num')} "
                f"sim={c.get('similarity')} hybrid={c.get('hybrid_score')} "
                f"sem={c.get('semantic_score')} key={c.get('keyword_score')}"
            )
            print(c.get("text", "")[:300].replace("\n", " "))
            print("---")
